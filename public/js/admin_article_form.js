...
class ReferenceList
{
    ...
    handleReferenceDelete(event) {
        ...
        $.ajax({
            url: '//localhost/symfony/elearningPlatform/SymfonyCasts/code-symfony-uploads/start/public/admin/article/references/'+id,
            method: 'DELETE'
        }).then(() => {
    ...
    render() {
        const itemsHtml = Array.from(this.references).map(reference => {
            return `
    ...
    <span class="drag-handle fa fa-reorder"></span>
     <input type="text" value="${reference.originalFilename}" class="form-control js-edit-filename" style="width: auto;">
        <span>
            <a href="//localhost/symfony/elearningPlatform/SymfonyCasts/code-symfony-uploads/start/public/admin/article/references/${reference.id}/download" class="btn btn-link btn-sm"><span class="fa fa-download"style="vertical-align: middle"></span></a>
